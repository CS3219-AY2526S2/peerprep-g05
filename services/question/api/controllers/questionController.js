...
// Shared helper: build WHERE clause from filters
...
// Validate question payload (shared between create & update)
...
// Helper: fetch test cases for given question IDs
...
// GET /  — paginated list with optional filters + text search
export async function getAllQuestions(req, res, next) {
    try {
        ...
        // Attach public test cases to each question
...
export async function getQuestionById(req, res, next) {
    try {
        ...
        // Include public test cases (add ?include_private=true for all)
...
export async function getRandomQuestion(req, res, next) {
    try {
        ...
        // Use TABLESAMPLE for large tables; fallback ORDER BY RANDOM() for small sets
        const result = await postgres.query(
...
export async function updateQuestion(req, res, next) {
    try {
        ...
        // Duplicate-title guard (excluding self)
        if (title) {
            const dup = await postgres.query(
        ...
        // Replace test cases if provided (delete old + insert new)
...
// GET /companies — list all distinct companies
