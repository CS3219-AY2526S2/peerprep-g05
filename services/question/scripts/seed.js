...
function inferProblemIntent(title, topics) {
    const t = String(title || "").toLowerCase();
    ...
    const topicHint = Array.isArray(topics) && topics.length > 0
        ? ` using ${topics.slice(0, 2).join(" and ")}`
        : "";
    return `Given the inputs for \"${title}\", return the required output according to the problem constraints${topicHint}.`;
}

function buildPythonBoilerplateDescription(title, topics, examples) {
    const [e1, e2, e3] = examples;
    const intent = inferProblemIntent(title, topics);
    return (
...
function buildPythonBoilerplateTestCases(title, topics = []) {
    ...
    return {
        description: buildPythonBoilerplateDescription(title, topics, examples),
        testCases,
    };
}
    ...
    try {
        ...
        for (const item of parsed) {
            ...
            const topics = Array.isArray(item?.topics) && item.topics.length > 0
                ? item.topics.map(String).map((x) => x.trim()).filter(Boolean).slice(0, 4)
                : ["Algorithms"];
            ...
            const boilerplate = buildPythonBoilerplateTestCases(title, topics);

            unique.push({
                title,
                description: legacy?.description || buildPythonBoilerplateDescription(title, topics, buildPythonBoilerplateExamples(title)),
                topics,
...
const BASE_QUESTIONS = [
    ...
    },
    {
        ...
        topics: ["Graphs", "Depth-First Search", "Breadth-First Search", "Matrix"],
        complexity: "Medium",
        companies: ["Amazon", "Google", "Meta", "Microsoft", "Bloomberg"],
        test_cases: [
    ...
    },
    {
        ...
        topics: ["Strings", "Breadth-First Search", "Hash Table"],
        complexity: "Hard",
    ...
    },
    {
        ...
        topics: ["Trees", "Design", "Depth-First Search", "Breadth-First Search"],
        complexity: "Hard",
    ...
    try {
        ...
        for (const [index, chunk] of questionChunks.entries()) {
            ...
            const placeholders = chunk.map((q, i) => {
                const offset = i * 5;
                values.push(q.title, q.description, q.topics, q.complexity, q.companies || []);
                return `($${offset + 1}, $${offset + 2}, $${offset + 3}, $${offset + 4}, $${offset + 5})`;
            });

            const inserted = await client.query(
                `INSERT INTO questions (title, description, topics, complexity, companies)
